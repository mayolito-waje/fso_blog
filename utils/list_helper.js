...
export const mostBlogs = (blogs) => {
  const blogsByAuthor = _.reduce(blogs, (list, blog) => {
  ...
  return blogsByAuthor.length > 0
    ? _.maxBy(blogsByAuthor, (l) => l.blogs)
    : {};
};

export const mostLikes = (blogs) => {
  const likesByAuthor = _.reduce(blogs, (list, blog) => {
    let authorInList = false;
  ...
  return likesByAuthor.length > 0
    ? _.maxBy(likesByAuthor, (l) => l.likes)
    : {};
};
